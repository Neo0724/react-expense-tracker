...
export default function ViewTransaction() {
    const { income, expenses, transactionMonth, setTransactionMonth } = useGlobalContext()

    let selectedDateExpense = expenses.filter(item => {

        if ( transactionMonth.expense === "all") return item

        return item.date.split("T")[0].split("-").reverse().join("-").split("-")[1].toString() === transactionMonth.expense
    })
    ...
    let selectedDateIncome = income.filter(item => {
        ...
        return item.date.split("T")[0].split("-").reverse().join("-").split("-")[1].toString() === transactionMonth.income
    })
  ...
  return (
    <div className="viewTransactionContainer">
        <div className="viewIncomeContainer">
            <div className="viewTransactionTitle">Income Transactions on : <MonthSelect category="income"/></div>
            <div className="viewIncomeDetailsContainer">
                {!selectedDateIncome.length ? <div className="empty">Empty ...</div> : null}
        ...
        <div className="viewExpensesContainer">
            <div className="viewTransactionTitle">Expense Transactions on : <MonthSelect category="expense"/></div>
            <div className="viewExpensesDetailsContainer">
                {!selectedDateExpense.length ? <div className="empty">Empty ...</div> : null}
                {selectedDateExpense.map(expenses => {
                    const randomID = crypto.randomUUID()
                    return <ExpenseContainer expenses={expenses} key={randomID} transactionMonth={ transactionMonth }/>
                })}
            </div>
        </div>
    </div>
