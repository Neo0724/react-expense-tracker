...
export function ExpenseContainer({ expenses, setExpenses }) {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${BASE_URL}/expense/delete-expense/${expenses._id}`, { headers: { authorization: cookies.access_token } })
      fetchExpenses()
    } catch (err) {
      console.log(err)
    }
...
export function IncomeContainer({ income, setIncome }) {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${BASE_URL}/income/delete-income/${income._id}`, { headers: { authorization: cookies.access_token } })
      fetchIncome()
    } catch (err) {
      console.log(err)
    }
